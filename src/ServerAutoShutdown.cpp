...
void ServerAutoShutdown::Init()
{
    ...
    {
        LOG_WARN("modules","> ServerAutoShutdown: Next time to shutdown < 10 seconds, Set next day", TimeToHumanReadable(nextResetTime).c_str());
        nextResetTime += DAY;
    }
    ...
    {
        LOG_ERROR("modules", "> ServerAutoShutdown: Ahah, how could this happen? Time to preannouce more 1 day? (%u). Set to 1 hour (3600)", preAnnounceSeconds);
